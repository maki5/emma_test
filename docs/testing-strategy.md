...
```
bulk-import-export/
├── testdata/                      # Pre-generated test data files
...
```
scripts/
├── test-complete-api.sh      # Full API test suite
├── test-imports.sh           # Import-specific tests
├── test-exports.sh           # Export-specific tests
├── test-validation.sh        # Validation and error handling
├── test-performance.sh       # Performance benchmarks
├── test-idempotency.sh       # Idempotency tests
├── test-cancellation.sh      # Cancellation API tests
└── utils/
    ├── setup.sh              # Environment setup
    ├── wait-for-job.sh       # Job completion helper
...
echo "✅ User import job started: $USER_JOB"
...
ARTICLE_JOB=$(curl -s -X POST \
  ...
  -F "resource=articles" \
  "${BASE_URL}/v1/imports" | jq -r '.job_id')
echo "✅ Article import job started: $ARTICLE_JOB"
...
COMMENT_JOB=$(curl -s -X POST \
  ...
  "${BASE_URL}/v1/imports" | jq -r '.job_id')
echo "✅ Comment import job (remote URL) started: $COMMENT_JOB"
...
echo "✅ Async export job started: $USER_EXPORT_JOB"

# Wait for completion and check status
sleep 5
echo "📊 Export job status:"
...
echo "🔄 Second request with same key (should return same job)..."
...
if [ "$FIRST_JOB" = "$SECOND_JOB" ]; then
  echo "✅ Idempotency working correctly: $FIRST_JOB"
else
  echo "❌ Idempotency failed: $FIRST_JOB != $SECOND_JOB"
  exit 1
fi
...
CANCEL_JOB=$(curl -s -X POST \
  ...
  -F "resource=users" \
  "${BASE_URL}/v1/imports" | jq -r '.job_id')
echo "✅ Started job for cancellation test: $CANCEL_JOB"
...
echo "🚀 Testing streaming export performance..."
START_TIME=$(date +%s)
...
if [ $DURATION -gt 0 ]; then
  ...
  echo "📊 Exported $RECORD_COUNT records in ${DURATION}s (${RPS} records/sec)"
else
  echo "📊 Exported $RECORD_COUNT records in <1s"
fi
